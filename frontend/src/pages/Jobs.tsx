...
export default function Jobs() {
  ...
  >
    ...
    <td className="py-2">
      {j.late_risk_label ? (
        <span
          className={`badge ${riskColors[j.late_risk_label]}`}
          title={`${j.late_risk_score}% predicted probability of being late`}
        >
          {j.late_risk_label} · {j.late_risk_score}%
        </span>
      ) : (
  ...
  const colCount = groupByClient ? 9 : 10
